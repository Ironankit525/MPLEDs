...
function KpiCard({ icon: Icon, iconBg, iconColor, label, value, sub, badge, badgeColor }) {
  return (
    <div className="group relative overflow-hidden rounded-none border border-slate-200/80 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
...
function KpiBar({ kpi, onExport }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
      ...
      {/* Quick Action card */}
      <div className="rounded-none border border-slate-200/80 bg-white p-4 shadow-sm">
      ...
      {/* Export Audit Report */}
      <div className="col-span-2 rounded-none border border-slate-200/80 bg-white p-4 shadow-sm md:col-span-2">
        <div className="flex items-center gap-2">
...
function ActionCenter({ items }) {
  ...
  return (
    <div className="bg-white border border-red-200 rounded-none shadow-sm mb-4 overflow-hidden">
        ...
        {items.map((item) => (
          <div key={item.id} className="px-4 py-4 flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              {item.submissionId && (
                <Link
                  to={`/contractor/submissions/${item.submissionId}`}
              ...
              <Link
                to={`/contractor/upload?workId=${item.workId}`}
              ...
              {filtered.map((w) => (
                <tr key={w.workId} className={w.riskLevel === 'HIGH' ? 'bg-red-50/40 transition-colors hover:bg-red-50' : 'transition-colors hover:bg-slate-50'}>
                  ...
                  <td className="px-4 py-3">
                    {w.submissionId ? (
                      <Link
                        to={`/contractor/submissions/${w.submissionId}`}
...
function ActivityFeed({ items }) {
  return (
    <div className="bg-white border border-slate-200 rounded-none shadow-sm p-4">
